...
def pred(ID) :
    ...
    global totalCount, rnaCount, dnaCount, bacCount
    totalCount = totalCount + 1
    if float(score[0,0]) >= float(score[0,1]) and float(score[0,0]) >= float(score[0,2]) :
        prediction = 'RNA'
        rnaCount = rnaCount + 1
    elif float(score[0,1]) > float(score[0,0]) and float(score[0,1]) > float(score[0,2]) :
        prediction = 'DNA'
        dnaCount = dnaCount + 1
    else :
        prediction = 'bacteria'
    ...
    print("The score is " + str(score) + ' and the prediciton is ' + prediction)
    print("The prediction rate is " + str(rnaCount) + "/" + str(dnaCount) + "/" + str(bacCount) + " with the total number of " +  str(totalCount))
...
writef = predF.write('\t'.join(['name', 'len', 'RNA_score', 'DNA_score','Prediction'])+'\n')
predF.close()
